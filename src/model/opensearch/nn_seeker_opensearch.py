...
class NnSeekerOpenSearch(NnSeeker):
    ...
    def __init__(self, config):
        ...
        self.url_embedding = (
            "http://localhost:8001/get-embedding"  # TODO: env variable? config?
        )
    ...
    def get_k_NN(self, item: ItemDto, k, filter_criteria) -> tuple[list, list]:
        ...
        try:
            ...
            request_payload = {"id": "no_id", "embedText": text_to_embed}
            response = httpx.post(
                self.url_embedding, json=request_payload, timeout=None
            ).json()
            embedding = response[
                "all-MiniLM-L6-v2"
            ]  # Todo: embedding = response[self.embedding_field_name]
