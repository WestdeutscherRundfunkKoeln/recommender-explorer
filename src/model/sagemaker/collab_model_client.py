...
class CollabModelClient(U2CSeeker):
    ...
    def __init__(self, config):
        ...
        cross_account_options = self.__get_cross_account_options(
            config[constants.MODEL_CONFIG_U2C][constants.MODEL_TYPE_U2C][
                "ARD-ALS-Experiments"
            ]["role_arn"]
        )
    ...
    def get_recos_user(
        self, user_item: UserItemDto, num_recos: int
    ) -> tuple[list, list, str]:
        json_body = json.dumps({"userId": user_item.id, "numberOfItems": num_recos})

        try:
            logger.info(
                "Invoking endpoint call to ["
            ...
            response = self.__sm_run_client.invoke_endpoint(
                EndpointName=self.__model_config["endpoint"].removeprefix(
                    "sagemaker://"
                ),
                Body=json_body,
            ...
            if response_data.get("message", "").startswith("Unknown userId"):
                raise UnknownUserError(
                    "Keine Empfehlungen für den Benutzer ["
                    + user_item.id
                    + "] in diesem Modell",
                    {},
                )
            ...
            logging.error(e)
            raise EndpointError(
                "Couldn't get a response from endpoint ["
                + self.__model_config["endpoint"]
                + "]",
                {},
            )
    ...
    def get_model_params(self):
        try:
            ...
            endpoint_details = self.__sm_client.describe_endpoint(
                EndpointName=self.__model_config["endpoint"].removeprefix(
                    "sagemaker://"
                )
            )
            endpoint_description = self.__sm_client.describe_endpoint_config(
            ...
            model_package_name = model_description["PrimaryContainer"][
                "ModelPackageName"
            ]
            model_package_description = self.__sm_client.describe_model_package(
            ...
            model_data_url = model_package_description["InferenceSpecification"][
                "Containers"
            ][0]["ModelDataUrl"]
            res = re.search(r"^.*\/(.*)/output/model.tar.gz$", model_data_url)
            train_job_descriptor = res.group(1)
            res = re.search("^(.*?)-", train_job_descriptor)
            execution_id = res.group(1)
            pipeline_arn = model_package_description["MetadataProperties"][
                "GeneratedBy"
            ]
            ...
            pipeline_execution_description = (
                self.__sm_client.describe_pipeline_definition_for_execution(
                    PipelineExecutionArn=pipeline_arn
                )
            )
            pipeline_definition = json.loads(
                pipeline_execution_description["PipelineDefinition"]
            )
            ...
            logger.warning(
                "Fehler beim Ermitteln der Modell-Parameter [" + str(e) + "]"
            )
            self.model_properties["Error"] = (
                "Es ist ein Fehler beim Ermitteln der Modellparameter aufgetreten"
            )
        ...
        for step in pipeline_definition["Steps"]:
            ...
            if step_name in self.step_reports.keys():
                ...
                for output in step_outputs:
                    if (
                        output["OutputName"]
                        == self.step_reports[step_name]["report-file"]
                    ):
                ...
                s3_response = self.__s3_client.get_object(
                    Bucket=o.netloc, Key=o.path.removeprefix("/")
                )

                s3_object_body = s3_response.get("Body")
                content = self.__reduce_dct(
                    json.loads(s3_object_body.read()), step_name
                )
    ...
    def __retrieve_training_properties(self, train_job_descriptor):
        trial_component_description = self.__sm_client.describe_trial_component(
            TrialComponentName=train_job_descriptor + "-aws-training-job"
        )
        trial_parameters = trial_component_description["Parameters"]

        self.model_properties["Trainings-Metriken"] = {}
        self.model_properties["Trainings-Metriken"]["Zielmetrik-Name"] = (
            trial_parameters["_tuning_objective_metric"]["StringValue"]
        )
        self.model_properties["Tuning-Hyperparameter"] = {}
        self.model_properties["Tuning-Hyperparameter"]["alpha"] = trial_parameters[
            "alpha"
        ]["NumberValue"]
        self.model_properties["Tuning-Hyperparameter"]["factors"] = trial_parameters[
            "factors"
        ]["NumberValue"]
        self.model_properties["Tuning-Hyperparameter"]["regularization"] = (
            trial_parameters["regularization"]["NumberValue"]
        )
        self.model_properties["Statische Hyperparameter"] = {}
        hpo_params = json.loads(trial_parameters["hpo_params"]["StringValue"])
        self.model_properties["Statische Hyperparameter"]["Coverage-Gewicht"] = (
            hpo_params["coverage_weight"]
        )
        self.model_properties["Statische Hyperparameter"]["Watchtime-Gewicht"] = (
            hpo_params["watchtime_weight"]
        )
        self.model_properties["Statische Hyperparameter"]["Random State"] = hpo_params[
            "random_state"
        ]
