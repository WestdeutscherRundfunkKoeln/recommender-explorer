...
class ClusteringModelClient:
    def __init__(self, config):
        ...
        cross_account_options = self.__get_cross_account_options(
            config[constants.MODEL_CONFIG_U2C]["clustering_models"]["U2C-Knn-Model"][
                "role_arn"
            ]
        )
    ...
    def get_user_cluster(self):
        ...
        try:
            logger.info(
                "Invoking clustering endpoint call to ["
    ...
    def get_users_by_category(self, genreCategory):
        body_dict = {
            "action": "filter_users",
            "params": {
                "filters": [{"column": "genreCategory", "value": genreCategory}]
            },
        }
        ...
        if not len(response_data):
            raise UnknownUserError(
                "Momentan gibt es keine Benutzer, die in erster Linie das Genre ["
                + genreCategory
                + "]"
                + " konsumieren",
                {},
