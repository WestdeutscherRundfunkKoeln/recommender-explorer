...
class RecommendationController():
    ...
    def get_items(self) -> tuple[list, list[list], str]:
        ...
        if self.get_display_mode() == constants.DISPLAY_MODE_SINGLE:
            ...
            for model in selected_models:
                ...
                self.set_model_and_strategy(model_info)
                hits, items, model_config = self.get_items_by_strategy_and_model(model_info)
                res.append(items[0])
    ...
    def _get_start_users_by_cluster(self, user_dto: UserItemDto, cluster_widget, start_idx, end_idx) -> tuple[int, list[UserItemDto]]:
    ...
    def _transpose_reco_filter_state(self, reco_filter, start_item):
        ...
        for filter_element in reco_filter.items():
            ...
            if action == 'termfilter':
                bool_terms = self._prepare_query_term_condition_statement(value, start_item, reco_filter, bool_terms)
            elif action == 'rangefilter':
                bool_terms = self._prepare_query_range_condition_statement(value, bool_terms)
            elif action == 'sort':
    ...
    def _prepare_query_term_condition_statement(self, value: list, start_item: ItemDto, reco_filter: dict, bool_terms: dict) -> dict:
        ...
        if logic != "choose":
            ...
            if field == "erzählweise":
                ...
                reco_filter['value_subgenreCategories'] = self.get_genres_and_subgenres_from_upper_category(reco_filter['value_inhaltCategory'], 'subgenres')
                field = 'subgenre'
        ...
        if len(transposed_values):
            term['terms'][self.FILTER_FIELD_MATRIX[field] + ".keyword"] = transposed_values
    ...
    def reset_component(self, component_group, component_label, to_value='default'):
        ...
        if self.watchers[component_group].get(component_label):
            ...
            component.param.watch(self.callbacks[component_group][component_label], 'value')
    ...
    def get_genres_and_subgenres_from_upper_category(self, selected_upper_categories, category): # category = 'genres' or 'subgenres'
        all_selected = []
