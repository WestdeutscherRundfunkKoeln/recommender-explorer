...
class RecommendationController:
    ...
    def __init__(self, config, current_client: str = ""):
        ...
        self.mapping_type = {
            "Verwandte Inhalte": "Semantic",
            "Diversität": "Diverse",
            "Aktualität": "Temporal",
        }
        self.mapping_direction = {
            "Ähnlicher": "more similar",
            "Aktueller": "more recent",
            "Weniger Diversität": "less diverse",
            "Mehr Diversität": "more diverse",
            "Weniger Aktualität": "less recent",
            "Mehr Aktualität": "more recent",
        }
    ...
    def set_mode(self):
        ...
        self.initial_model_info = self.config[self.model_config][self.model_type][
            self.selected_models[0]
        ]
    ...
    def _get_start_items_c2c_s2c(self, model: dict) -> tuple[int, list[ItemDto]]:
        ...
        provenance = (
            ...
            else constants.ITEM_PROVENANCE_S2C
        )
    ...
    def _get_start_items(self, model: dict) -> tuple[int, list[ItemDto]]:
        ...
        """
        if (
            self.model_type == constants.MODEL_TYPE_C2C
            or self.model_type == constants.MODEL_TYPE_S2C
        ):
    ...
    def _get_reco_items_c2c_s2c(self, start_item: ItemDto, model: dict):
        ...
        return (
            self.item_accessor.get_items_by_ids(
                item_dto, kidxs[: self.num_NN], model_type
            )[0],
    ...
    def _check_crid(self, crid_field):
        if not isinstance(crid_field.value.strip(), str):
            raise ValueError(
                "Crid must be a string"
            )  # elif not crid_field.value.strip().startswith("crid://"):  #     raise ValueError("Id must be of format crid://")
    ...
    def _get_active_start_components(self) -> list:
        ...
        if self.model_type == constants.MODEL_TYPE_U2C:
            ...
            return list(
                filter(lambda x: x.visible, self.components["search_choice"].values())
            )
        else:
            ...
            if label == "refinementType":
                ...
                if direction:
                    ...
                    refinement_direction = (
                        self.refinement_widget.map_refinement_direction(
                            mapped_direction
                        )
                    )
    ...
    def reset_component(self, component_group, component_label, to_value="default"):
        ...
        if component_label == "refinementType":
            self.refinement_widget.reset_refinement_widget(
                self._get_refinement_widget()
            )
        if to_value == "default":
            component.value = component.params["reset_to"]
        else:
