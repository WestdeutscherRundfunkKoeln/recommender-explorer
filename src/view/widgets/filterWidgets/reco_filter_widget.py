...
class RecoFilterWidget(FilterWidget):
    def create(self, config: dict[str, Any]) -> pn.widgets.MultiSelect | None:
        ...
        action_list = []
        action_2_list = []
        if config.get("visible_action"):
            for action_1 in config.get("visible_action"):
                action_list.append(action_1.get("action_name"))
        if config.get("visible_action_2"):
            for action_2 in config.get("visible_action_2"):
