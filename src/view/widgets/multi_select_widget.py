...
from view.widgets.filterWidgets.model_choice_widget import ModelChoiceWidget
from view.widgets.filterWidgets.item_filter_widget import ItemFilterWidget
from view.widgets.filterWidgets.upper_item_filter import UpperItemFilterWidget
from view.widgets.filterWidgets.reco_filter_widget import RecoFilterWidget
...
class MultiSelectionWidget(UIWidget):
    def get_multi_select_options(
        ...
        if multi_select_config.get(c.MULTI_SELECT_OPTIONS_KEY):
            ...
            option_dictionary = {}
            for option in multi_select_config.get(
                c.MULTI_SELECT_DICTIONARY_OPTIONS_KEY, ""
            ):
            ...
            option_list = self.controller_instance.get_item_defaults(
                multi_select_config.get(c.MULTI_SELECT_OPTIONS_DEFAULT_FUNCTION_KEY)
            )
            return option_list, []
        return [], []

    def build_multi_select_widget(
        self, multi_select_config
    ...
    def create(self, config: dict[str, Any]) -> pn.widgets.MultiSelect | None:
        ...
        if multi_select_register_value == "item_filter":
            item_filter_widget = ItemFilterWidget(
                self, self.reco_explorer_app_instance, self.controller_instance
            )
            return item_filter_widget.create(config)
        elif multi_select_register_value == "upper_item_filter":
            upper_item_filter_widget = UpperItemFilterWidget(
                self, self.reco_explorer_app_instance, self.controller_instance
            )
            return upper_item_filter_widget.create(config)
        elif multi_select_register_value == "reco_filter":
            reco_filter_widget = RecoFilterWidget(
                self, self.reco_explorer_app_instance, self.controller_instance
            )
            return reco_filter_widget.create(config)
        elif multi_select_register_value == "model_choice":
            model_choice_widget = ModelChoiceWidget(
                self, self.reco_explorer_app_instance, self.controller_instance
            )
            return model_choice_widget.create(config)
        else:
