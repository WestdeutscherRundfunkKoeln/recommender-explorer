...
class UserCard:
    ...
    def draw(self, user_dto: UserItemDto, nr, model, model_config, modal_func):
        ...
        card = pn.Card(
            styles={
                "background": self.config[model_config][user_dto.provenance][model][
                    "start_color"
                ]
            },
            margin=5,
        ...
        if user_dto.last_active:
            last_active = datetime.fromtimestamp(
                user_dto.last_active / 1000.0
            ).strftime("%d/%m/%Y, %H:%M:%S")
        else:
            last_active = "- unbekannt -"

        if user_dto.history:
            history_button = pn.widgets.Button(
                name="Historie anzeigen", button_type="primary"
            )
            history_button.params = {"item": user_dto, "button": "history_button"}
            history_button.on_click(modal_func)
            card_optional_objects.append(history_button)

        if 1:  # user_dto.model_params:
            params_button = pn.widgets.Button(
                name="Modellparameter anzeigen", button_type="primary"
            )
            params_button.params = {"item": user_dto, "button": "params_button"}
